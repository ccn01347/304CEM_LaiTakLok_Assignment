...
(function($) {
    ...
    // Edited by Lai Tak Lok
    function base64ToPng(bytes){
            return 'data:image/png;base64,' + bytes;
         }

    function fetchJsonUrls(urls){
        console.log(urls);
        ...
        $.when.apply(undefined, ajaxCalls).then(function() {
            ...
            for (var i = 0; i < arguments.length; i++){
                bytesArray.push(arguments[i][0].stickers[0].image_data);
            }

            var html = '';
            for (var i = 0; i < bytesArray.length; i++){
                var src = base64ToPng(bytesArray[i]);
                html += '<div class="swiper-slide swiper-slide-duplicate" style="width: 217.5px; margin-right: 20px;"><img class="img-fluid" src="';
                html += src;
                html += '" alt="alternative"></div>'
            }


            document.getElementById("swiper-wrapper-sticker").innerHTML += html;
     ...
     function websiteInit(){
        $.ajax({
            url: "http://localhost:4000/api/publicstickers",
            type: "GET",
            dataType: "JSON",
            success: function(result){
                console.log(result);
                if (result.status_code === 200){
                    fetchJsonUrls(result.data);
                }
            },
